...
import {
  ...
  type ScenarioResult,
} from "@/lib/calculators/salary-vs-dividend";
...
function ScenarioCard({
  ...
  return (
    <div
      className={`rounded-xl border p-4 ${highlight ? "bg-primary-900 border-primary-900 text-white" : "border-border bg-white"}`}
    >
      <p
        className={`text-xs font-semibold tracking-wide uppercase ${highlight ? "text-primary-300" : "text-muted-foreground"}`}
      >
        {scenario.label}
      </p>
      <p
        className={`mt-2 font-serif text-3xl font-semibold tabular-nums ${highlight ? "text-accent-300" : "text-primary-900"}`}
      >
        {fmt(scenario.takeHome)}
      </p>
      <p
        className={`mt-0.5 text-xs ${highlight ? "text-primary-300" : "text-muted-foreground"}`}
      >
        take-home
      </p>

      <div
        className={`mt-3 space-y-1 border-t pt-3 text-xs ${highlight ? "border-primary-700" : "border-border"}`}
      >
        {[
          ...
          <div key={row.label} className="flex justify-between">
            <span
              className={
                highlight ? "text-primary-300" : "text-muted-foreground"
              }
            >
              {row.label}
            </span>
            <span
              className={`tabular-nums ${highlight ? "text-white" : "text-primary-900"}`}
            >
              {fmt(row.value)}
            </span>
          </div>
        ...
        <div
          ...
          style={{
            borderColor: highlight ? "rgba(255,255,255,0.2)" : undefined,
          }}
        >
          <span className={highlight ? "text-primary-200" : "text-primary-900"}>
            Effective rate
          </span>
          <span
            className={`tabular-nums ${highlight ? "text-accent-300" : "text-primary-900"}`}
          >
...
export function SalaryVsDividendForm() {
  ...
  const results = calculateSalaryVsDividend({
  ...
  useEffect(() => {
    trackEvent("calculator_viewed", {
      calculator_slug: "salary-vs-dividend-calculator",
    });
  }, []);
    ...
    <div>
      ...
      <div className="mb-6 grid max-w-lg gap-4 sm:grid-cols-2">
        ...
        <div>
          ...
          <p className="text-muted-foreground mb-1.5 text-xs">
            Personal contributions (reduces taxable income)
          </p>
          <div className="relative">
            <span className="text-muted-foreground pointer-events-none absolute top-1/2 left-3 -translate-y-1/2">
              £
            ...
            <input
              ...
              value={pension}
              onChange={(e) => {
                setPension(Math.max(0, Number(e.target.value)));
                handleChange();
              }}
              className="border-border text-primary-900 focus:border-primary-500 focus:ring-primary-500/20 w-full rounded-lg border bg-white py-2.5 pr-3 pl-8 text-base focus:ring-2 focus:outline-none"
            />
      ...
      <p className="text-muted-foreground mt-4 text-xs">
        * Optimal scenario assumes director salary at secondary NI threshold
        (£9,100) with remaining profit distributed as dividends. Employment
        Allowance not claimed (sole director).
      </p>
