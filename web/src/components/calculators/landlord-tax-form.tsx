...
import {
  calculateLandlordTax,
  type LandlordTaxInput,
} from "@/lib/calculators/landlord-tax";
...
export function LandlordTaxForm() {
  ...
  const [ownershipType, setOwnershipType] =
    useState<LandlordTaxInput["ownershipType"]>("personal");
  ...
  useEffect(() => {
    trackEvent("calculator_viewed", {
      calculator_slug: "landlord-tax-calculator",
    });
  }, []);
            ...
            <div className="grid grid-cols-2 gap-2">
              {(["personal", "company"] as const).map((type) => (
                ...
                >
                  ...
                  />
                  <span className="text-primary-900 text-sm font-medium capitalize">
                    {type}
                  </span>
                </label>
              ))}
          ...
          {[
            {
              ...
              value: rentalIncome,
              set: setRentalIncome,
              helper: "Gross rent received",
            },
            {
              id: "ltf-mortgage",
              label: "Annual mortgage interest",
              value: mortgageInterest,
              set: setMortgageInterest,
              helper: "Finance costs (interest only, not capital repayment)",
            },
            {
              id: "ltf-expenses",
              label: "Other allowable expenses",
              value: otherExpenses,
              set: setOtherExpenses,
              helper: "Letting fees, insurance, repairs, accountancy",
            },
              ...
              ? [
                  {
                    ...
                    value: otherIncome,
                    set: setOtherIncome,
                    helper: "Salary, pension, or other non-rental income",
                  },
                ]
              : []),
        ...
        >
          ...
          {ownershipType === "personal" && mortgageInterest > 0 && (
            <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900">
              <strong>Section 24 applies.</strong> Mortgage interest (£
              {mortgageInterest.toLocaleString()}) cannot be deducted from
              profits directly. You receive a 20% tax relief credit (£
              {fmt(result.mortgageInterestRelief)}) instead, which is less
