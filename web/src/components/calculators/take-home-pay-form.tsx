...
export function TakeHomePayForm() {
  ...
  const result = calculateTakeHomePay({ grossSalary, pensionPercent, studentLoanPlan });

  useEffect(() => {
    trackEvent("calculator_viewed", { calculator_slug: "take-home-pay-calculator" });
  }, []);

  const handleChange = () => {
    if (!hasInteracted) {
      setHasInteracted(true);
      trackEvent("calculator_started", { calculator_slug: "take-home-pay-calculator" });
    }
  };
    ...
    <div>
      ...
      <div className="grid gap-8 lg:grid-cols-5">
        <div className="lg:col-span-2 space-y-4">
          <div>
            <label htmlFor="thp-salary" className="text-primary-900 mb-1 block text-sm font-semibold">Gross annual salary</label>
            <div className="relative">
              ...
              <input
                ...
                step={1000}
                value={grossSalary}
                onChange={(e) => { setGrossSalary(Math.max(0, Number(e.target.value))); handleChange(); }}
          ...
          <div>
            <label htmlFor="thp-pension" className="text-primary-900 mb-1 block text-sm font-semibold">
              Employee pension contribution <span className="text-muted-foreground font-normal">(%)</span>
            </label>
            <p className="text-muted-foreground mb-1.5 text-xs">Your personal contribution as % of gross salary</p>
            <div className="relative">
              <input
                id="thp-pension"
                type="number"
                ...
                onChange={(e) => { setPensionPercent(Math.min(100, Math.max(0, Number(e.target.value)))); handleChange(); }}
                className="border-border text-primary-900 focus:border-primary-500 focus:ring-primary-500/20 w-full rounded-lg border bg-white py-2.5 px-3 pr-8 text-base focus:ring-2 focus:outline-none"
              />
              <span className="text-muted-foreground pointer-events-none absolute right-3 top-1/2 -translate-y-1/2">%</span>
            </div>
          </div>

          <div>
            <label htmlFor="thp-student-loan" className="text-primary-900 mb-1.5 block text-sm font-semibold">Student loan</label>
            <select
              id="thp-student-loan"
              value={studentLoanPlan}
              onChange={(e) => { setStudentLoanPlan(e.target.value as StudentLoanPlan); handleChange(); }}
            ...
            >
              {studentLoanOptions.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
            </select>
          </div>
        </div>
        ...
        <div className="lg:col-span-3 space-y-4" aria-live="polite">
          <div className="bg-primary-900 rounded-xl p-5 text-white">
            ...
            <p className="text-accent-300 mt-1 font-serif text-4xl font-semibold tabular-nums">{fmtDec(result.netMonthly)}</p>
            <p className="text-primary-300 mt-1 text-sm">{fmt(result.netAnnual)} per year · {fmt(result.netWeekly)} per week</p>
          </div>

          <div className="border-border rounded-xl border p-5">
            <h3 className="text-primary-900 mb-3 text-sm font-semibold">Deductions breakdown</h3>
            ...
            <div className="bg-primary-50 border-primary-100 mt-4 rounded-lg border p-3 text-xs">
              <span className="text-primary-900 font-medium">Effective tax rate: </span>
              <span className="text-muted-foreground">{result.effectiveRate.toFixed(1)}% of gross salary</span>
            </div>
          </div>
        </div>
      </div>
