...
export function calculateDividendTax(
  ...
  const remainingHigher = Math.max(
    0,
    higherBand - Math.max(0, nonDivTaxable - basicBand),
  );
  ...
  if (divBasic > 0)
    breakdown.push({
      label: `Basic rate (${(DIVIDENDS.basicRate * 100).toFixed(2)}%)`,
      value: taxBasic,
      rate: `${(DIVIDENDS.basicRate * 100).toFixed(2)}%`,
    });
  if (divHigher > 0)
    breakdown.push({
      label: `Higher rate (${(DIVIDENDS.higherRate * 100).toFixed(2)}%)`,
      value: taxHigher,
      rate: `${(DIVIDENDS.higherRate * 100).toFixed(2)}%`,
    });
  if (divAdditional > 0)
    breakdown.push({
      label: `Additional rate (${(DIVIDENDS.additionalRate * 100).toFixed(2)}%)`,
      value: taxAdditional,
      rate: `${(DIVIDENDS.additionalRate * 100).toFixed(2)}%`,
    });
  breakdown.push({
  ...
  return {
    ...
    dividendTax,
    effectiveRate:
      dividendIncome > 0 ? (dividendTax / dividendIncome) * 100 : 0,
    breakdown,
  };
}
