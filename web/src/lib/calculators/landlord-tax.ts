...
export function calculateLandlordTax(
  input: LandlordTaxInput,
): LandlordTaxResult {
  const {
    rentalIncome,
    mortgageInterest,
  ...
  if (input.ownershipType === "company") {
    ...
    return {
      ...
      effectiveRentalTaxRate:
        netRentalProfit > 0 ? (corpTax / netRentalProfit) * 100 : 0,
  ...
  const higherOther = Math.min(
    Math.max(0, otherTaxable - basicBand),
    higherBand,
  );
  const additionalOther = Math.max(0, otherTaxable - basicBand - higherBand);
  const otherIncomeTax =
    basicOther * INCOME_TAX.basicRate +
    higherOther * INCOME_TAX.higherRate +
    additionalOther * INCOME_TAX.additionalRate;
  ...
  const mortgageInterestRelief = Math.min(
    mortgageInterest * SECTION_24.reliefRate,
    taxOnRentalProfit,
  );
  ...
  return {
    ...
    netTaxOnRental,
    effectiveRentalTaxRate:
      rentalProfit > 0 ? (netTaxOnRental / rentalProfit) * 100 : 0,
    totalIncome,
    breakdown: [
      { label: "Rental income", value: rentalIncome },
      ...
      {
        label: "Section 24 mortgage relief (20%)",
        value: -mortgageInterestRelief,
      },
      {
        label: "Net tax on rental income",
        value: netTaxOnRental,
        isTotal: true,
      },
    ],
  };
...
export function calculatePropertyCgt(
  ...
  return {
    gain,
    annualExemption: CGT.annualExemption,
    taxableGain,
    cgtDue,
    rate,
  };
