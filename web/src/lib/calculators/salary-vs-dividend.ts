...
function calcCorpTax(profit: number): number {
  ...
  if (profit <= CORPORATION_TAX.smallProfitsLimit) return profit * CORPORATION_TAX.smallProfitsRate;
  if (profit >= CORPORATION_TAX.mainRateLimit) return profit * CORPORATION_TAX.mainRate;
  return profit * CORPORATION_TAX.mainRate - CORPORATION_TAX.marginalReliefFraction * (CORPORATION_TAX.mainRateLimit - profit);
}

function calcIncomeTax(income: number, dividends: number, pension: number): number {
  ...
  const incTax = basicInc * INCOME_TAX.basicRate + higherInc * INCOME_TAX.higherRate + additionalInc * INCOME_TAX.additionalRate;

  // Dividend tax
  const remainingBasic = Math.max(0, basicBand - incTaxable);
  const remainingHigher = Math.max(0, higherBand - Math.max(0, incTaxable - basicBand));
  ...
  const divTax = divBasic * DIVIDENDS.basicRate + divHigher * DIVIDENDS.higherRate + divAdditional * DIVIDENDS.additionalRate;
...
export function calculateSalaryVsDividend(input: SalaryVsDividendInput): {
  ...
  function buildScenario(label: string, salary: number): ScenarioResult {
    ...
    const profitAfterSalaryAndNi = Math.max(0, companyProfit - salary - employerNi);
    ...
    return { label, salary, dividends, corporationTax: corpTax, incomeTax, employeeNi, employerNi, totalTax, takeHome, effectiveRate };
  }

  const optimal = buildScenario("Optimal (low salary + dividends)", OPTIMAL_DIRECTOR_SALARY);
