...
export function calculatePersonalTax(
  ...
  const nonDividendIncome =
    employmentIncome + selfEmploymentProfit + rentalIncome;
  ...
  const excessOverTaper = Math.max(
    0,
    adjustedNetIncome - INCOME_TAX.taperStart,
  );
  const personalAllowance = Math.max(
    0,
    INCOME_TAX.personalAllowance - Math.floor(excessOverTaper / 2),
  );

  // Non-dividend taxable income
  const nonDivTaxable = Math.max(
    0,
    nonDividendIncome - pensionContributions - personalAllowance,
  );
  ...
  const higherNonDiv = Math.min(
    Math.max(0, nonDivTaxable - basicBandWidth),
    higherBandWidth,
  );
  const additionalNonDiv = Math.max(
    0,
    nonDivTaxable - basicBandWidth - higherBandWidth,
  );
  ...
  const dividendAboveAllowance = Math.max(
    0,
    dividendIncome - DIVIDENDS.allowance,
  );
  const remainingBasic = Math.max(0, basicBandWidth - nonDivTaxable);
  const remainingHigher = Math.max(
    0,
    higherBandWidth - Math.max(0, nonDivTaxable - basicBandWidth),
  );

  const divBasic = Math.min(dividendAboveAllowance, remainingBasic);
  const divHigher = Math.min(
    Math.max(0, dividendAboveAllowance - divBasic),
    remainingHigher,
  );
  const divAdditional = Math.max(
    0,
    dividendAboveAllowance - divBasic - divHigher,
  );
  ...
  const incomeTax =
    basicRateTax + higherRateTax + additionalRateTax + dividendTax;
  ...
  const niMain = Math.max(
    0,
    Math.min(
      niableIncome - NI_EMPLOYEE.primaryThreshold,
  ...
  const ni =
    Math.max(0, niMain) * NI_EMPLOYEE.mainRate +
    niUpper * NI_EMPLOYEE.upperRate;
  ...
  if (studentLoanPlan !== "none") {
    ...
    studentLoanRepayment =
      Math.max(0, niableIncome - plan.threshold) * plan.rate;
  }
